     ...
     if ( Tdomain%specel(n)%Orient_Edges(i) == 0 ) then
        ...
        case (0)
           do j = 1,ngll-2
              Tdomain%sEdge(ne)%MassMat(j) = Tdomain%sEdge(ne)%MassMat(j) + &
                   Tdomain%specel(n)%MassMat(ngll-1-j,0,0)
           enddo
        case (1)
           do j = 1,ngll-2
              Tdomain%sEdge(ne)%MassMat(j) = Tdomain%sEdge(ne)%MassMat(j) + &
                   Tdomain%specel(n)%MassMat(ngllx-1,ngll-1-j,0)
           enddo
        case (2)
           do j = 1,ngll-2
              Tdomain%sEdge(ne)%MassMat(j) = Tdomain%sEdge(ne)%MassMat(j) + &
                   Tdomain%specel(n)%MassMat(ngll-1-j,nglly-1,0)
           enddo
        case (3)
           do j = 1,ngll-2
              Tdomain%sEdge(ne)%MassMat(j) = Tdomain%sEdge(ne)%MassMat(j) + &
                   Tdomain%specel(n)%MassMat(0,ngll-1-j,0)
           enddo
        case (4)
           do j = 1,ngll-2
              Tdomain%sEdge(ne)%MassMat(j) = Tdomain%sEdge(ne)%MassMat(j) + &
                   Tdomain%specel(n)%MassMat(ngllx-1,0,ngll-1-j)
           enddo
        case (5)
           do j = 1,ngll-2
              Tdomain%sEdge(ne)%MassMat(j) = Tdomain%sEdge(ne)%MassMat(j) + &
                   Tdomain%specel(n)%MassMat(ngll-1-j,0,ngllz-1)
           enddo
        case (6)
           do j = 1,ngll-2
              Tdomain%sEdge(ne)%MassMat(j) = Tdomain%sEdge(ne)%MassMat(j) + &
                   Tdomain%specel(n)%MassMat(0,0,ngll-1-j)
           enddo
        case (7)
           do j = 1,ngll-2
              Tdomain%sEdge(ne)%MassMat(j) = Tdomain%sEdge(ne)%MassMat(j) + &
                   Tdomain%specel(n)%MassMat(ngllx-1,nglly-1,ngll-1-j)
           enddo
        case (8)
           do j = 1,ngll-2
              Tdomain%sEdge(ne)%MassMat(j) = Tdomain%sEdge(ne)%MassMat(j) + &
                   Tdomain%specel(n)%MassMat(ngllx-1,ngll-1-j,ngllz-1)
           enddo
        case (9)
           do j = 1,ngll-2
              Tdomain%sEdge(ne)%MassMat(j) = Tdomain%sEdge(ne)%MassMat(j) + &
                   Tdomain%specel(n)%MassMat(ngll-1-j,nglly-1,ngllz-1)
           enddo
        case (10)
           do j = 1,ngll-2
              Tdomain%sEdge(ne)%MassMat(j) = Tdomain%sEdge(ne)%MassMat(j) + &
                   Tdomain%specel(n)%MassMat(0,nglly-1,ngll-1-j)
           enddo
        case (11)
           do j = 1,ngll-2
              Tdomain%sEdge(ne)%MassMat(j) = Tdomain%sEdge(ne)%MassMat(j) + &
                   Tdomain%specel(n)%MassMat(0,ngll-1-j,ngllz-1)
           enddo
        end select
        ...
        if (Tdomain%sEdge(ne)%PML) then
           ...
           case (0)
              do j = 1,ngll-2
                 Tdomain%sEdge(ne)%DumpMass(j,:) = Tdomain%sEdge(ne)%DumpMass(j,:) + &
                      Tdomain%specel(n)%DumpMass(ngll-1-j,0,0,:)
                 if (Tdomain%sEdge(ne)%FPML) then
           ...
           case (1)
              do j = 1,ngll-2
                 Tdomain%sEdge(ne)%DumpMass(j,:) = Tdomain%sEdge(ne)%DumpMass(j,:) + &
                      Tdomain%specel(n)%DumpMass(ngllx-1,ngll-1-j,0,:)
                 if (Tdomain%sEdge(ne)%FPML) then
                    Tdomain%sEdge(ne)%Ivx(j) = Tdomain%sEdge(ne)%Ivx(j) + &
           ...
           case (2)
              do j = 1,ngll-2
                 Tdomain%sEdge(ne)%DumpMass(j,:) = Tdomain%sEdge(ne)%DumpMass(j,:) + &
                      Tdomain%specel(n)%DumpMass(ngll-1-j,nglly-1,0,:)
                 if (Tdomain%sEdge(ne)%FPML) then
                    Tdomain%sEdge(ne)%Ivx(j) = Tdomain%sEdge(ne)%Ivx(j) + &
           ...
           case (3)
              do j = 1,ngll-2
                 Tdomain%sEdge(ne)%DumpMass(j,:) = Tdomain%sEdge(ne)%DumpMass(j,:) + &
                      Tdomain%specel(n)%DumpMass(0,ngll-1-j,0,:)
                 if (Tdomain%sEdge(ne)%FPML) then
                    Tdomain%sEdge(ne)%Ivx(j) = Tdomain%sEdge(ne)%Ivx(j) + &
           ...
           case (4)
              do j = 1,ngll-2
                 Tdomain%sEdge(ne)%DumpMass(j,:) = Tdomain%sEdge(ne)%DumpMass(j,:) + &
                      Tdomain%specel(n)%DumpMass(ngllx-1,0,ngll-1-j,:)
                 if (Tdomain%sEdge(ne)%FPML) then
                    Tdomain%sEdge(ne)%Ivx(j) = Tdomain%sEdge(ne)%Ivx(j) + &
           ...
           case (5)
              do j = 1,ngll-2
                 Tdomain%sEdge(ne)%DumpMass(j,:) = Tdomain%sEdge(ne)%DumpMass(j,:) + &
                      Tdomain%specel(n)%DumpMass(ngll-1-j,0,ngllz-1,:)
                 if (Tdomain%sEdge(ne)%FPML) then
                    Tdomain%sEdge(ne)%Ivx(j) = Tdomain%sEdge(ne)%Ivx(j) + &
           ...
           case (6)
              do j = 1,ngll-2
                 Tdomain%sEdge(ne)%DumpMass(j,:) = Tdomain%sEdge(ne)%DumpMass(j,:) + &
                      Tdomain%specel(n)%DumpMass(0,0,ngll-1-j,:)
                 if (Tdomain%sEdge(ne)%FPML) then
                    Tdomain%sEdge(ne)%Ivx(j) = Tdomain%sEdge(ne)%Ivx(j) + &
           ...
           case (7)
              do j = 1,ngll-2
                 Tdomain%sEdge(ne)%DumpMass(j,:) = Tdomain%sEdge(ne)%DumpMass(j,:) + &
                      Tdomain%specel(n)%DumpMass(ngllx-1,nglly-1,ngll-1-j,:)
                 if (Tdomain%sEdge(ne)%FPML) then
                    Tdomain%sEdge(ne)%Ivx(j) = Tdomain%sEdge(ne)%Ivx(j) + &
           ...
           case (8)
              do j = 1,ngll-2
                 Tdomain%sEdge(ne)%DumpMass(j,:) = Tdomain%sEdge(ne)%DumpMass(j,:) + &
                      Tdomain%specel(n)%DumpMass(ngllx-1,ngll-1-j,ngllz-1,:)
                 if (Tdomain%sEdge(ne)%FPML) then
                    Tdomain%sEdge(ne)%Ivx(j) = Tdomain%sEdge(ne)%Ivx(j) + &
           ...
           case (9)
              do j = 1,ngll-2
                 Tdomain%sEdge(ne)%DumpMass(j,:) = Tdomain%sEdge(ne)%DumpMass(j,:) + &
                      Tdomain%specel(n)%DumpMass(ngll-1-j,nglly-1,ngllz-1,:)
                 if (Tdomain%sEdge(ne)%FPML) then
                    Tdomain%sEdge(ne)%Ivx(j) = Tdomain%sEdge(ne)%Ivx(j) + &
           ...
           case (10)
              do j = 1,ngll-2
                 Tdomain%sEdge(ne)%DumpMass(j,:) = Tdomain%sEdge(ne)%DumpMass(j,:) + &
                      Tdomain%specel(n)%DumpMass(0,nglly-1,ngll-1-j,:)
                 if (Tdomain%sEdge(ne)%FPML) then
                    Tdomain%sEdge(ne)%Ivx(j) = Tdomain%sEdge(ne)%Ivx(j) + &
           ...
           case (11)
              do j = 1,ngll-2
                 Tdomain%sEdge(ne)%DumpMass(j,:) = Tdomain%sEdge(ne)%DumpMass(j,:) + &
                      Tdomain%specel(n)%DumpMass(0,ngll-1-j,ngllz-1,:)
                 if (Tdomain%sEdge(ne)%FPML) then
                    Tdomain%sEdge(ne)%Ivx(j) = Tdomain%sEdge(ne)%Ivx(j) + &
