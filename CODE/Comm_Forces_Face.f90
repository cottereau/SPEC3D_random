...
do i = 0,Tdomain%sComm(n)%nb_faces-1
  ...
  if ( Tdomain%sComm(n)%orient_faces(i) == 0 ) then
    ...
    if (Tdomain%sFace(nf)%PML) then
      do j = 1,Tdomain%sFace(nf)%ngll2-2
          do k = 1,Tdomain%sFace(nf)%ngll1-2
              Tdomain%sFace(nf)%Forces1(ngll2-1-j,k,0:2) = Tdomain%sFace(nf)%Forces1(ngll2-1-j,k,0:2) + &
                                                   Tdomain%sComm(n)%TakeForcesPML(ngllPML,1,0:2)
              Tdomain%sFace(nf)%Forces2(ngll2-1-j,k,0:2) = Tdomain%sFace(nf)%Forces2(ngll2-1-j,k,0:2) + &
                                                   Tdomain%sComm(n)%TakeForcesPML(ngllPML,2,0:2)
              Tdomain%sFace(nf)%Forces3(ngll2-1-j,k,0:2) = Tdomain%sFace(nf)%Forces3(ngll2-1-j,k,0:2) + &
                                                   Tdomain%sComm(n)%TakeForcesPML(ngllPML,3,0:2)
              ngllPML = ngllPML + 1
          enddo
      enddo
    ...
    if (Tdomain%sFace(nf)%PML) then
      do j = 1,Tdomain%sFace(nf)%ngll2-2
          do k = 1,Tdomain%sFace(nf)%ngll1-2
              Tdomain%sFace(nf)%Forces1(j,ngll1-1-k,0:2) = Tdomain%sFace(nf)%Forces1(j,ngll1-1-k,0:2) + &
                                                   Tdomain%sComm(n)%TakeForcesPML(ngllPML,1,0:2)
              Tdomain%sFace(nf)%Forces2(j,ngll1-1-k,0:2) = Tdomain%sFace(nf)%Forces2(j,ngll1-1-k,0:2) + &
                                                   Tdomain%sComm(n)%TakeForcesPML(ngllPML,2,0:2)
              Tdomain%sFace(nf)%Forces3(j,ngll1-1-k,0:2) = Tdomain%sFace(nf)%Forces3(j,ngll1-1-k,0:2) + &
                                                   Tdomain%sComm(n)%TakeForcesPML(ngllPML,3,0:2)
              ngllPML = ngllPML + 1
          enddo
      enddo
    ...
    if (Tdomain%sFace(nf)%PML) then
      do j = 1,Tdomain%sFace(nf)%ngll2-2
          do k = 1,Tdomain%sFace(nf)%ngll1-2
              Tdomain%sFace(nf)%Forces1(ngll2-1-j,ngll1-1-k,0:2) = Tdomain%sFace(nf)%Forces1(ngll2-1-j,ngll1-1-k,0:2) + &
                                                   Tdomain%sComm(n)%TakeForcesPML(ngllPML,1,0:2)
              Tdomain%sFace(nf)%Forces2(ngll2-1-j,ngll1-1-k,0:2) = Tdomain%sFace(nf)%Forces2(ngll2-1-j,ngll1-1-k,0:2) + &
                                                   Tdomain%sComm(n)%TakeForcesPML(ngllPML,2,0:2)
              Tdomain%sFace(nf)%Forces3(ngll2-1-j,ngll1-1-k,0:2) = Tdomain%sFace(nf)%Forces3(ngll2-1-j,ngll1-1-k,0:2) + &
                                                   Tdomain%sComm(n)%TakeForcesPML(ngllPML,3,0:2)
              ngllPML = ngllPML + 1
          enddo
      enddo
